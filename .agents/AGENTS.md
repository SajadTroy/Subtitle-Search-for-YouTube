...
- **`inject.js`**: A script injected directly into the Main World of the page. Because content scripts live in an Isolated World, they cannot directly read internal player states or intercept page-level network calls. This script handles:
  ...
  - Main-world multi-source caption track discovery (`cachedPlayerResponses`, `movie_player.getPlayerResponse()`, `movie_player.getOption('captions', 'tracklist')`, `ytd-watch-flexy`/`grid` DOM data, `window.ytInitialPlayerResponse`, and direct HTML watch page fetch fallback).
  - Main-world same-origin proxy fetching for subtitle timed text across formats (`raw`, `fmt=json3`, `fmt=srv1`, `fmt=vtt`).
...
- **2026-09-01**: Resolved subtitle loading failures on video playback and intermittent UI window visibility issues. Enhanced `inject.js` with InnerTube `/youtubei/v1/player` interception, multi-source caption discovery, main-world proxy fetching, and player caption triggering. Updated `content.js` with continuous DOM connection health checks, multi-selector fallback injection, full HTML entity decoding, and multi-language track prioritization. Verified zero comments in code per extension guidelines.
